...
export default function U1Studio() {
  ...
  const [tagline, setTagline] = useState('Capturing Fleeting Raw Emotions');
  const [description, setDescription] = useState('Award-winning cinematography and portraiture. We freeze raw human bonds and grand architectures with cinematic lens systems.');
  ...
  useEffect(() => {
    ...
    const unsubDiv = onSnapshot(doc(db, 'divisions', 'u1-studio'), (snap) => {
      if (snap.exists()) {
        const d = snap.data();
          ...
          {shutterActive && (
            ...
            >
              ...
              <motion.div 
                ...
                className="absolute top-0 bottom-0 left-0 w-1/2 bg-navy-dark border-r border-gold-accent/20"
              />
              {/* Shutter Blade Right */}
              <motion.div 
                initial={{ x: '100%' }}
                ...
                exit={{ x: '100%' }}
                transition={{ duration: 0.35, ease: 'easeInOut' }}
                className="absolute top-0 bottom-0 right-0 w-1/2 bg-navy-dark border-l border-gold-accent/20"
              />
            </motion.div>
          )}
        </AnimatePresence>
